...
// ★재정의: items.id 는 재등록 시 바뀌어 키로 못 씀 → (입찰일|사건번호|법원) 3키.
...
// 3키 정규화 키 (in_date 는 yyMMdd 정규화)
function _dmKey3_(inDate, sakun, court) {
  return _dmDateKey6_(inDate) + '|' + String(sakun || '').trim() + '|' + String(court || '').trim();
}
...
function _ensureDamulgeonHeader_(sheet) {
  ...
  // 구버전(link_item_id 키, in_date 없음) → items 에서 3키(in_date/sakun_no/court) 복구 (보조필드 보존)
  if (oldObjs.some(function(o){ return o.link_item_id && !o.in_date; })) {
    var byId = {};
    try {
      var ish = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName(DB_SHEET_NAME);
      if (ish && ish.getLastRow() > 1) {
        ish.getRange(2, 1, ish.getLastRow() - 1, 4).getValues().forEach(function(r){
          byId[String(r[0])] = { in_date: formatParamsDate(r[1]), sakun_no: String(r[2] || ''), court: String(r[3] || '') };
        });
      }
    } catch (e) {}
    oldObjs.forEach(function(o){
      if (o.link_item_id && !o.in_date && byId[o.link_item_id]) { var it = byId[o.link_item_id]; o.in_date = it.in_date; o.sakun_no = it.sakun_no; o.court = it.court; }
    });
  }
  // 새 3키 모두 있는 행만 유지 (복구 불가/빈 행은 폐기)
  oldObjs = oldObjs.filter(function(o){ return String(o.in_date || '').trim() && String(o.sakun_no || '').trim() && String(o.court || '').trim(); });
  sheet.clear();
  ...
  sheet.getRange(1, 1, sheet.getMaxRows(), 3).setNumberFormat('@');   // A~C(3키: in_date/sakun_no/court) 텍스트 강제
}
...
function getDamulgeonList() {
  try {
    ...
    // damulgeon 시트 보조필드(명의/향/인증/비고) 머지 — 3키(in_date|sakun_no|court)
...
function saveDamulgeonItemBid(inDate, sakun, court, bidprice) {
  var __lock = LockService.getScriptLock();
  try { __lock.waitLock(15000); } catch (e) { return { success: false, message: '저장이 혼잡합니다.' }; }
  try {
    var sheet = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName(DB_SHEET_NAME);
    if (!sheet || sheet.getLastRow() < 2) return { success: false, message: 'items 없음' };
    var key = _dmKey3_(inDate, sakun, court);
    var data = sheet.getRange(2, 1, sheet.getLastRow() - 1, 8).getValues();   // A~H (id..bidprice)
    for (var i = 0; i < data.length; i++) {
      var r = data[i];
      if (_dmKey3_(formatParamsDate(r[1]), r[2], r[3]) === key) {
        sheet.getRange(i + 2, 8).setValue(String(bidprice || ''));   // H(8) = bidprice
        SpreadsheetApp.flush();
        return { success: true, id: String(r[0] || ''), bidprice: String(bidprice || '') };
      }
    }
    return { success: false, message: '해당 물건(3키) 없음' };
  } catch (e) { Logger.log('[saveDamulgeonItemBid] ' + e); return { success: false, message: String(e) }; }
  finally { __lock.releaseLock(); }
}
