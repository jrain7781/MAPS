...
배포 완료 시 반드시 다음 형식을 준수합니다:
1. 푸쉬: 이전과 같음/다름 - scriptId [ID]
2. 배포: @[버전] / 이전과 같음/다름-[배포ID]
...
| 2026-03-18 | 일괄 적용(입찰가 요청) 로직 리팩토링 및 모달 동적 제목 적용 |
| 2026-03-18 | 입찰가 3자리 콤마 표시 기능 추가 및 테이블 컬럼 너비 최적화 |
| 2026-03-18 | 히스토리 중복 로그 방지(flush 적용) 및 전달항목(chuchen_state) 수동 기록 복구 |
